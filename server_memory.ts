...
export function formatSystemInstructionsWithMemories(baseInstruction: string, memories: Memory[]): string {
  ...
  let memoryBlock = 
    "\n\n" +
    "=== MOYNA PERSISTENT MEMORY CORE (RECOLLECTIONS) ===\n" +
...
export async function processConversationSlice(
  ...
  incomingMemories?: Memory[]
): Promise<{ updatedMemories: Memory[]; transactions: MemoryTransaction[] } | null> {
  if (isConsolidating) {
  ...
  try {
    ...
    const currentMemories = incomingMemories || await loadMemories();
    ...
    const dialogueContext = dialogueHistory.map(line => `${line.role === "user" ? "User" : "Moyna"}: ${line.text}`).join("\n");

    const prompt = `You are Moyna's deep cognitive recollection engine. Your task is to analyze the recent conversation piece against previous persistent memories, and output precise update transactions.
...
- ACTIONS:
  ...
  - "REMOVE": If a memory was explicitly disproven or the user directly asked Moyna to forget it.
- TEXT STYLE: Express the memories as clean, concise, third-person declarative summaries (e.g., 'The user's developer/boss is Ratan Boss.', 'The user loves playing GTA 6.', 'The user enjoys technical and fast-paced styling explanations.'). Do not include conversational filler, quotes, or timestamps.
- ID: For ADD, leave blank. For UPDATE or REMOVE, provide the exact 'id' from the "Current user memories" list.`;
    ...
    // Only write to system local JSON if memories are not coming from client session
    if (!incomingMemories) {
      await saveMemories(updatedMemories);
    }

    isConsolidating = false;
    return { updatedMemories, transactions };
